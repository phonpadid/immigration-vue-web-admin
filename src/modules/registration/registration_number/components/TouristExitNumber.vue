...
const { getTourists_exit, incrementTouristsExit, decrementTouristsExit } =
  useRegistrationnumberStore();
const touristsExitData = ref<NumberResponse>({
  per_day: 0,
  per_mouth: 0,
  per_year: 0,
});
...
const fetchTouristsExitData = async () => {
  try {
    const data = await getTourists_exit();
    touristsExitData.value = data;
  } catch (error) {
    console.error("Error fetching tourists exit data:", error);
  }
};
...
const handleOk = async () => {
  ...
  try {
    if (modalType.value === "increment") {
      await incrementTouristsExit(value);
    } else {
      await decrementTouristsExit(value);
    }
    showModal.value = false;
    await fetchTouristsExitData();
  } catch (error) {
