...
const { getRegiter_exit, incrementRegisterExit, decrementRegisterExit } =
  useRegistrationnumberStore();

const exitRegisterData = ref<NumberResponse>({
  per_day: 0,
  per_mouth: 0,
  per_year: 0,
});
...
const fetchExitData = async () => {
  try {
    const data = await getRegiter_exit();
    exitRegisterData.value = data;
  } catch (error) {
    console.error("Error fetching exit register data:", error);
...
const handleOk = async () => {
  ...
  try {
    if (modalType.value === "increment") {
      await incrementRegisterExit(value);
    } else {
      await decrementRegisterExit(value);
    }
