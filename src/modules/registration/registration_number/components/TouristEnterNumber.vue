...
const { getTourists_enter, incrementTouristsEnter, decrementTouristsEnter } =
  useRegistrationnumberStore();
const touristsEnterData = ref<NumberResponse>({
  per_day: 0,
  per_mouth: 0,
  per_year: 0,
});
...
const fetchTouristsEnterData = async () => {
  try {
    const data = await getTourists_enter();
    touristsEnterData.value = data;
  } catch (error) {
    console.error("Error fetching tourists enter data:", error);
  }
};
...
const handleOk = async () => {
  ...
  try {
    if (modalType.value === "increment") {
      await incrementTouristsEnter(value);
    } else {
      await decrementTouristsEnter(value);
    }
    showModal.value = false;
    await fetchTouristsEnterData();
  } catch (error) {
    ...
    >
      ...
      <div class="w-full space-y-2">
        ...
        <UiInput
          ...
          type="number"
          :placeholder="modalType === 'increment' ? 'ປ້ອນຈຳນວນ' : 'ປ້ອນຈຳນວນ'"
          class="w-full"
          size="large"
        />
      </div>
