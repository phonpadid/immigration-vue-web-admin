...
import type { LanguageKey, ApiLangKey } from "../interface/nationality.interface";
...
const NATIONALITY_HEADER = "ສັນຊາດທັງໝົດ";
const ADD_BUTTON_TEXT = "ເພີ່ມສັນຊາດ";
...
const nationality_add = () => {
  push({ name: "nationalities_add" });
};
...
const currentData = computed(() => {
  ...
  return nationalityStore.nationalitiesByLang[apiLang]?.data || [];
});

// computed property สำหรับ total items
const totalItems = computed(() => {
  const apiLang = getApiLangKey(activeTab.value);
  return nationalityStore.nationalitiesByLang[apiLang]?.total || 0;
});
...
const handleSelect = (key: string, record: any) => {
  if (key === "1") {
    push({ name: "nationalities_edit", params: { id: record.id } });
  } else if (key === "2") {
    Modal.confirm({
      title: "ຢືນຢັນການລົບ",
      ...
      onOk: async () => {
        try {
          Loading.value = true;
          await nationalityStore.deleteNationality(record.id);
          await loadData();
    ...
    <Tab :tabs="tabsConfig" v-model:activeKey="activeTab" class="mb-4">
      ...
      <template v-for="tab in tabsConfig" :key="tab.key" #[tab.slotName]>
        ...
        <Table
          ...
          :loading="nationalityStore.isLoading"
          :pagination="{
