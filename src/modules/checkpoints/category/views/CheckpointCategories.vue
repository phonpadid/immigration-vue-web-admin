...
const addCheckpointCategoires = () => {
  push({ name: "checkpointCategories_add" });
};
/********************************************************************************* */
...
const handleSelect = (key: string, record: any) => {
  ...
  if (key === "1") {
    // View details
    push({ name: "checkpointCategories_edit", params: { id: record.id } });
  } else if (key === "2") {
    Modal.confirm({
      title: "ຢືນຢັນການລົບ",
      ...
      onOk: async () => {
        try {
          Loading.value = true;
          await deleteCheckpointCategories(record.id);
...
<template>
  ...
  <div class="relative">
    <Table
      :columns="columns"
      :dataSource="checkpointCategories.data || []"
